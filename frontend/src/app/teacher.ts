...
export class TeacherPage {
  ...
  async refresh() {const id = this.detail()?.classInfo.id; if(id) this.detail.set(await this.api.request<Detail>('/classes/' + id));}
  open(c: ClassInfo) {void this.run(async () => {this.detail.set(await this.api.request<Detail>('/classes/' + c.id)); this.proposal.set(null); this.selected.set(null); this.links.set([]); this.tab.set('students'); this.history.set(await this.api.request<any[]>('/classes/'+c.id+'/proposals'));});}
